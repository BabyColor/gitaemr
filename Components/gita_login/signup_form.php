<?
// Signup Form
echo "<form action=index.php?mod=gita_login&job=2 method=POST><table><tr>"; //---Draw form
...
while($g=mysqli_fetch_assoc($fgroups->sq)){
	...
	while($i=mysqli_fetch_assoc($su_field->sq)){
		...
		$viewsonic="field_visible_edit"; //-----Check which kind of view (Edit or View)
		...
		if($input_type=='select'){ //---Decide how to draw the field based on the input_type
			...
			$field_type="<input name=". $i['field_id'] ."' list='". $i['field_id'] ."'><datalist id='". $i['field_id'] ."'>";
